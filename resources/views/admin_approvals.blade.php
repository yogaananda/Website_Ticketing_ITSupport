...
<div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto space-y-8">
    ...
    <div class="mb-4 border-b border-gray-200">
        <ul class="flex flex-wrap -mb-px text-sm font-medium text-center" id="procurement-tab" data-tabs-toggle="#procurement-tab-content" role="tablist">
            <li class="me-2" role="presentation">
                <button class="inline-block p-4 border-b-2 rounded-t-lg transition-colors" id="pending-tab" data-tabs-target="#pending" type="button" role="tab" aria-controls="pending" aria-selected="false">Menunggu Persetujuan</button>
            </li>
            <li class="me-2" role="presentation">
                <button class="inline-block p-4 border-b-2 rounded-t-lg transition-colors hover:text-gray-600 hover:border-gray-300" id="history-tab" data-tabs-target="#history" type="button" role="tab" aria-controls="history" aria-selected="false">Riwayat Arsip</button>
            </li>
        </ul>
    </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 align-top whitespace-nowrap">
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border {{ $item->prioColor }}">
                ...
                <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
                    <div>
                        <h3 class="text-lg font-bold text-gray-900 flex items-center gap-2">
                            ...
                            Arsip Persetujuan Pembelian
                        </h3>
                        <p class="text-xs text-gray-500">Riwayat pengajuan yang sudah diproses (Disetujui/Ditolak).</p>
                    </div>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Pelapor</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Detail Barang</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Harga Total</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Status & Keputusan</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @forelse($historyProcurements as $item)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 align-top">
                                    <div class="text-sm font-semibold text-gray-900">{{ $item->user->full_name }}</div>
                                    <div class="text-[10px] text-gray-400 mt-0.5">{{ $item->created_at->format('d M Y') }}</div>
                                ...
                                <td class="px-6 py-4 align-top whitespace-nowrap">
                                    <div class="text-sm font-bold text-gray-900">Rp {{ number_format($item->estimated_price * $item->quantity, 0, ',', '.') }}</div>
                                </td>
                                <td class="px-6 py-4 align-top">
                                    @if($item->status == 'approved')
                                        ...
                                        <div class="text-xs text-gray-500 mt-1">oleh Admin pada {{ $item->updated_at->format('d M Y') }}</div>
